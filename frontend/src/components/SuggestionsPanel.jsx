          ...
          <ul className="space-y-3 text-sm text-slate-800">
            {list.map((it, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <span className="mt-1 shrink-0 inline-flex items-center justify-center h-6 w-6 rounded-full bg-blue-50 text-blue-600 font-semibold">{idx+1}</span>
                <div className="flex-1">
                  {typeof it === 'string' ? (
                    <div className="text-slate-800">{it}</div>
                  ) : (
                    <pre className="whitespace-pre-wrap text-slate-800">{JSON.stringify(it, null, 2)}</pre>
                  )}
                </div>
              </li>
            ))}
    ...
    return (
      <div className="mt-6">
        ...
        <div className="bg-white rounded-md p-4 border border-slate-100 text-sm text-slate-800">
          {/* Match summary if present */}
          {typeof suggestions.matched_pct !== 'undefined' || typeof suggestions.match_pct !== 'undefined' || typeof suggestions.fit_score !== 'undefined' ? (
            <div className="mb-3 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div className="px-2 py-1 rounded-md text-sm font-semibold bg-slate-100 text-slate-800">
                  {typeof suggestions.matched_pct !== 'undefined' ? `${Math.round(suggestions.matched_pct)}%` : (typeof suggestions.match_pct !== 'undefined' ? `${Math.round(suggestions.match_pct)}%` : (typeof suggestions.fit_score !== 'undefined' ? `${Math.round(suggestions.fit_score)}%` : 'N/A'))}
                </div>
              </div>
              <div className="w-48 bg-slate-100 h-2 rounded-full overflow-hidden">
                <div
                  className={`h-full ${((suggestions.matched_pct || suggestions.match_pct || suggestions.fit_score) >= 75) ? 'bg-emerald-500' : ((suggestions.matched_pct || suggestions.match_pct || suggestions.fit_score) >= 40 ? 'bg-yellow-500' : 'bg-red-500')}`}
                  style={{ width: `${Math.min(100, suggestions.matched_pct || suggestions.match_pct || suggestions.fit_score || 0)}%` }}
                />
              </div>
            </div>
          ) : null}
